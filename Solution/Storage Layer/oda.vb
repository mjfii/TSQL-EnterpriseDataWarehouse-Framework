...
#Region "CLR Exposed Methods"
    ...
    Public Shared Sub BuildModel(ByVal DatabaseName As SqlString,
...
#Region "ARA Methods"
    ...
    Private Shared Sub ProcessCallToBuild(ByVal SqlCnn As SqlConnection,
        ...
        If ModelToProcess.EntityCount = 0 Then PrintClientMessage("• There are zero entities to process...") : Exit Sub
    ...
    Private Shared Sub BuildValidModel(ByVal ModelToProcess As Store,
        ...
        Try
            ...
            If ProcessAbstractsInFull And ModelToProcess.EntityCount > 0 Then
                ...
                Entities = ModelToProcess.NotDeletedEntities
        ...
        Dim min As Integer = CInt(DateDiff(DateInterval.Minute, StartTime, ed))
        Dim sec As Integer = CInt(DateDiff(DateInterval.Second, StartTime, ed) Mod 60)
    ...
    Private Shared Sub PrintEstimatedChanges(ByVal ModelToProcess As Store)
        ...
        For Each Entity In ModelToProcess.Entities
            ...
            If Entity.CreateEntity Then
                PrintClientMessage(New String(Space, 2) & "-> " & Entity.TableName.ToString & New String(Char.Parse(Dot), 36 - Entity.TableName.ToString.Length) & "...[CREATE]")

            ElseIf Entity.DeleteEntity Then
                PrintClientMessage(New String(Space, 2) & "-> " & Entity.TableName.ToString & New String(Char.Parse(Dot), 36 - Entity.TableName.ToString.Length) & ".....[DROP]")

            ElseIf Not Entity.HasChange Then
                PrintClientMessage(New String(Space, 2) & "-> " & Entity.TableName.ToString & New String(Char.Parse(Dot), 36 - Entity.TableName.ToString.Length) & "[NO CHANGE]")

            ElseIf EntityAttributes IsNot Nothing Then
                PrintClientMessage(New String(Space, 2) & "-> " & Entity.TableName.ToString & New String(Char.Parse(Dot), 36 - Entity.TableName.ToString.Length) & "....[ALTER]")
    ...
    Private Shared Function GetMetadata(ByVal DatabaseName As String, _
        ...
        ReturnInternalResults(GetMetadata, InstanceQuery, "ara_instance_properties", DatabaseConnection)

        ' this command the passed in database name
        ExecuteDDLCommand("use [" & DatabaseName & "];", DatabaseConnection)

        ReturnInternalResults(GetMetadata, DatabaseQuery, "ara_database_properties", DatabaseConnection)
        ReturnInternalResults(GetMetadata, EntityQuery, "ara_entity_definition", DatabaseConnection)
        ReturnInternalResults(GetMetadata, AttributeQuery, "ara_attribute_definition", DatabaseConnection)
        ReturnInternalResults(GetMetadata, AbstractQuery, "ara_abstract_definition", DatabaseConnection)
        ReturnInternalResults(GetMetadata, AbstractColumnQuery, "ara_abstract_column_definition", DatabaseConnection)
        ReturnInternalResults(GetMetadata, SecurityQuery, "ara_security_definition", DatabaseConnection)
...
#Region "Model Variables"
        ...
        Private _database_compatibility As EDW.Common.SQLServerCompatibility
...
#Region "Model Properties"
        ...
        ''' <summary>Returns the entity based on an index number.</summary>
        ...
        ''' <summary>Returns then entity based on the entity name.</summary>
        ...
        ''' <summary></summary>
        Protected Friend ReadOnly Property NotDeletedEntities As Entity()
            Get
                Return (From de As Entity In _entities Where de.DeleteEntity = False Select de).ToArray
            End Get
        End Property
...
#Region "Model Constants"
        ' misc strings
        Private Const Spacer As Char = " "c
        Private Const EmptyString As String = ""
        Private Const Null As String = "null"
        Private Const NotNull As String = "not null"

        ' table names
        Private Const ara_database_properties As String = "ara_database_properties"
        Private Const ara_entity_definition As String = "ara_entity_definition"
        Private Const ara_attribute_definition As String = "ara_attribute_definition"
        Private Const ara_abstract_definition As String = "ara_abstract_definition"
        Private Const ara_abstract_column_definition As String = "ara_abstract_column_definition"
        ...
        Private Const ara_entity_type As String = "ara_entity_type"
        Private Const ara_hash_large_objects As String = "ara_hash_large_objects"
        Private Const ara_new_entity_type As String = "ara_new_entity_type"
        Private Const ara_entity_create As String = "ara_entity_create"
        Private Const ara_entity_delete As String = "ara_entity_delete"

        ' attribute
        Private Const ara_attribute_name As String = "ara_attribute_name"
        Private Const ara_attribute_referenced_entity As String = "ara_attribute_referenced_entity"
        Private Const ara_attribute_referenced_entity_type As String = "ara_attribute_referenced_entity_type"
        Private Const ara_attribute_datatype As String = "ara_attribute_datatype"
        Private Const ara_attribute_default As String = "ara_attribute_default"
        Private Const ara_attribute_ordinal As String = "ara_attribute_ordinal"
        Private Const ara_attribute_sort As String = "ara_attribute_sort"
        Private Const ara_attribute_optional As String = "ara_attribute_optional"
        Private Const ara_attribute_business_identifier As String = "ara_attribute_business_identifier"
        Private Const ara_attribute_description As String = "ara_attribute_description"
        Private Const ara_attribute_add_column As String = "ara_attribute_add_column"
        Private Const ara_attribute_delete_column As String = "ara_attribute_delete_column"
        Private Const ara_attribute_alter_column As String = "ara_attribute_alter_column"
        Private Const ara_attribute_alter_foreign_key As String = "ara_attribute_alter_foreign_key"
        Private Const ara_attribute_alter_default As String = "ara_attribute_alter_default"
        Private Const ara_attribute_alter_alternate_key As String = "ara_attribute_alter_alternate_key"

        Private Const ara_security_definition As String = "ara_security_definition"
        ...
        Private Const ara_abstract_name As String = "ara_abstract_name"
        Private Const ara_abstract_description As String = "ara_abstract_description"
        Private Const ara_abstract_predicate As String = "ara_abstract_predicate"

#End Region

#Region "Model Constructors"

        ''' <summary>Used to construct new ara model.</summary>
        ''' <param name="ModelDataset">A DataSet containing five (5) tables for each of the ARA's components.</param>
        Protected Friend Sub New(ByVal ModelDataset As DataSet)
            ...
            Try
                ...
                _database_compatibility = DatabaseAttributes.Item("compatibility_level").ToString.StringToDatabaseCompatibility
            Catch ex As Exception
            ...
            Try
                ...
                For Each SecurityRow In SecurityTable.Rows
                    _security.AddMember(New Security(SecurityRow(ara_abstract_security_group).ToString, SecurityRow(ara_abstract_security_role).ToString))
                Next
                ...
                For Each EntityRow In EntityTable.Rows
                    ...
                    For Each AttributeRow In AttributeRows
                        ...
                        NewEntity.NewAttribute(AttributeRow(ara_attribute_name).ToString,
                                               ...
                                               CInt(AttributeRow(ara_attribute_ordinal)),
                    ...
                    ' check for a couple model invalidations
                    ...
                    For Each AbstractRow In AbstractRows
                        ...
                        AbstractColumnRows = AbstractColumnTable.Select(String.Format(ara_abstract_column_select_string,
                                                                                      EntityName,
                                                                                      AbstractRow(ara_abstract_security_group).ToString,
...
#Region "Entity Variables"
            ...
            Private _entity_type As EDW.Common.EntityType
            Private _new_entity_type As EDW.Common.EntityType
            Private _hash_large_objects As EDW.Common.YesNoType
...
#Region "Entity Properties"
            ...
            ''' <value></value>
            ''' <returns></returns>
            Protected ReadOnly Property EntityType As EDW.Common.EntityType
                Get
                    Return _entity_type
                End Get
            End Property
            ...
            ''' <value></value>
            ''' <returns></returns>
            Protected Friend ReadOnly Property HashLargeObjects As EDW.Common.YesNoType
                Get
            ...
            ''' <value></value>
            ''' <returns></returns>
            Protected Friend ReadOnly Property NewEntityType As EDW.Common.EntityType
                Get
            ...
            Protected Friend ReadOnly Property CreateEntityDefinition As String
                Get
                    ...
                    If EntityType = EDW.Common.EntityType.TypeI Then
            ...
            Protected Friend ReadOnly Property AddColumnDefinition As String
                Get
                    ...
                    Dim NewColumns As EntityAttribute() = (From nc As EntityAttribute In _attribute
                                                           Where nc.BusinessIdentifier = EDW.Common.YesNoType.Yes Or nc.AddColumn
                                                           Select nc).ToArray()
                    ...
                    For Each NewColumn In NewColumns
                        ...
                        ReturnString = Replace(ReturnString, "{{{optionality}}}", If(NewColumn.Optionality = EDW.Common.YesNoType.No, NotNull, Null))
                    ...
                    NewColumns = (From nc As EntityAttribute In _attribute
                                  Where (nc.BusinessIdentifier = EDW.Common.YesNoType.Yes Or nc.AddColumn) And nc.ReferencedEntity <> ""
                                  Select nc).ToArray()
            ...
            Protected Friend ReadOnly Property AlterColumnDefinition As String
                Get
                    ...
                    For Each AlteredColumn In AlteredColumns
                        ...
                        ReturnString = Replace(ReturnString, "{{{optionality}}}", If(AlteredColumn.Optionality = EDW.Common.YesNoType.No, NotNull, Null))
            ...
            Protected Friend ReadOnly Property ControlInsertDefinition As String
                Get
                    ...
                    ReturnString = Replace(ReturnString, "{{{hash prefix}}}", If(HashLargeObjects = EDW.Common.YesNoType.No, "hashbytes(N'sha1',", "[dbo].[ara_hash]("))
                    Return ReturnString
                End Get
            End Property
            ...
            Protected Friend ReadOnly Property ForeignKeyDefinition(ByVal ColumnSet As EntityAttribute()) As String
                Get
                    ...
                    For Each Column In ColumnSet
                        ...
                        ReturnString = Replace(ReturnString, "{{{referenced entity type}}}", If(Column.ReferencedEntityType = EDW.Common.EntityType.TypeI, EmptyString, ".Keystore"))
            ...
            Protected Friend ReadOnly Property UniqueAttributes As EntityAttribute()
                Get
                    ...
                    Return (From ua As EntityAttribute In _attribute Where ua.BusinessIdentifier = EDW.Common.YesNoType.Yes And Not ua.DeleteColumn Order By ua.Ordinal Select ua).ToArray
                End Get
            End Property
            ...
            Protected Friend ReadOnly Property AtomicAttributes As EntityAttribute()
                Get
                    ...
                    Return (From aa As EntityAttribute In _attribute Where aa.BusinessIdentifier = EDW.Common.YesNoType.No And Not aa.DeleteColumn Order By aa.Ordinal Select aa).ToArray
                End Get
            End Property
            ...
            Protected Friend ReadOnly Property AttributeMetadataDefintion As String
                Get
                    ...
                    For Each ea In Attributes
                        ...
                        ep = Replace(ep, "{{{value}}}", ea.Ordinal.ToString)
                        ep += vbCrLf
                ...
                Get
                    ...
                    For Each Column In ColumnSet
                        ReturnString += New String(Spacer, Padding) & Column.ColumnName & Spacer & Column.Datatype & Spacer & _
                                        If(Column.Optionality = EDW.Common.YesNoType.Yes, "null,", "not null,") & vbCrLf
                    Next Column
...
#Region "Entity Constructors"
            ...
            Protected Friend Sub New(ByVal Entity As String,
                                     ...
                                     ByVal EntityType As EDW.Common.EntityType,
                                     ByVal HashLargeObjects As EDW.Common.YesNoType,
                                     ByVal NewEntityType As EDW.Common.EntityType,
            ...
            Protected Friend Sub NewAttribute(ByVal Attribute As String,
                                              ...
                                              ByVal ReferencedEntityType As EDW.Common.EntityType,
                                              ByVal Datatype As String,
                                              ByVal DefaultValue As String,
                                              ByVal Ordinal As Integer,
                                              ByVal SortOrder As EDW.Common.SortOrderType,
                                              ByVal Optionality As EDW.Common.YesNoType,
                                              ByVal BusinessIdentifier As EDW.Common.YesNoType,
                                              ByVal Description As String,
                ...
                Try
                    ...
                    If BusinessIdentifier = EDW.Common.YesNoType.Yes Then _has_business_key = True
                    ...
                    If ((AddColumn Or AlterColumn) And DefaultValue = "" And Optionality = EDW.Common.YesNoType.No) And Not _create_entity Then
                    ...
                    If String.Compare(Attribute, Attribute.ToUpper, False) = 0 Then
                        Warnings.AddMember("Entity attribute [" & Entity & "].[" & Attribute & "] is in upper case. Recommend proper case, e.g. CustomerNumber.")
                    End If

                    If String.Compare(Attribute, Attribute.ToLower, False) = 0 Then
...
#Region "Attribute Variables"
                ...
                Private _referenced_entity_type As EDW.Common.EntityType
                Private _datatype As String
                Private _default_value As String
                Private _ordinal As Integer
                Private _sort_order As EDW.Common.SortOrderType
                Private _optionality As EDW.Common.YesNoType
                Private _business_identifier As EDW.Common.YesNoType
                Private _description As String
...
#Region "Attribute Properties"
                ...
                ''' <summary></summary>
                Protected Friend ReadOnly Property ReferencedEntityType As EDW.Common.EntityType
                    Get
                ...
                ''' <summary></summary>
                Protected Friend ReadOnly Property SortOrder As EDW.Common.SortOrderType
                    Get
                        Return _sort_order
                    End Get
                End Property
                ...
                ''' <summary></summary>
                Protected Friend ReadOnly Property Optionality As EDW.Common.YesNoType
                    Get
                        Return _optionality
                    End Get
                End Property
                ...
                ''' <summary></summary>
                Protected Friend ReadOnly Property BusinessIdentifier As EDW.Common.YesNoType
                    Get
...
#Region "Attribute Contructors"
                ...
                Protected Friend Sub New(ByVal Name As String, _
                                         ...
                                         ByVal ReferencedEntityType As EDW.Common.EntityType, _
                                         ByVal Datatype As String, _
                                         ByVal DefaultValue As String, _
                                         ByVal Ordinal As Integer,
                                         ByVal SortOrder As EDW.Common.SortOrderType, _
                                         ByVal Optionality As EDW.Common.YesNoType, _
                                         ByVal BusinessIdentifier As EDW.Common.YesNoType, _
            ...
            Protected Friend Class Abstract
                ...
                Protected Friend ReadOnly Property CreateDefinition As String
                    Get
                        ...
                        If _entity_type = EDW.Common.EntityType.TypeI Then
                            ReturnString += My.Resources.ARA_AbstractDefinitionTypeI & vbCrLf
                        ElseIf _entity_type = EDW.Common.EntityType.TypeII Then
